---
name: cron
description: 仅在需要未来定时执行或周期执行任务时，使用本 skill。使用 ltclaw-gy-x cron list/create/get/state/pause/resume/delete/run 管理任务，并始终显式传入 --agent-id。
metadata:
...
4. 显式带上 --agent-id
5. ltclaw-gy-x cron create 创建任务
...
- 缺少参数时，先问用户再创建
- 修改/暂停/删除前，先 `ltclaw-gy-x cron list --agent-id <agent_id>`
- 排查问题时，用 `ltclaw-gy-x cron state <job_id> --agent-id <agent_id>`
