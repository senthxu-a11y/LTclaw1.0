...
- **Working directory:** Edit skill files directly under `$QWENPAW_WORKING_DIR`
  (default `~/.ltclaw-gy-x`), including `$QWENPAW_WORKING_DIR/skill_pool/` and each
...
- **Conflict handling:** If save, import, upload, or broadcast would land on a
  name that already exists, LTCLAW-GY.X returns a conflict instead of silently
...
1. **Import built-ins**.
   ...
   | **QA_source_index**           | Internal LTCLAW-GY.X source/doc index skill for quickly mapping keywords to source paths and local docs.                            | Built-in                                                       |
   | **cron**                      | Scheduled jobs. Create, list, pause, resume, or delete jobs via `ltclaw-gy-x cron` or Console **Control → Cron Jobs**.              | Built-in                                                       |
   ...
   | **guidance**                  | Answer LTCLAW-GY.X installation and configuration questions by consulting local docs first.                                         | Built-in                                                       |
   ...
   **out-of-date**. Use **Update Built-in Skills** to add missing built-ins
   ...
   [CLI](./cli) (`ltclaw-gy-x cron`) or Console **Control → Cron Jobs**:

   - Create: `ltclaw-gy-x cron create --type agent --name "xxx" --cron "0 9 * * *" ...`
   - List: `ltclaw-gy-x cron list`
   - Check state: `ltclaw-gy-x cron state <job_id>`
...
`$QWENPAW_WORKING_DIR/workspaces/{agent_id}/skills/`, including using LTCLAW-GY.X itself to help
...
variables, declare them in `metadata.requires`; LTCLAW-GY.X exposes them as
...
workspace and channel, LTCLAW-GY.X injects that config into the runtime environment
...
For `requires` metadata, the parser checks keys in order: `metadata.openclaw.requires` → `metadata.ltclaw-gy-x.requires` → `metadata.requires`. The first one found is used.
