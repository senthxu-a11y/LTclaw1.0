...
### LTCLAW-GY.X vs OpenClaw: Feature Comparison

Please check the [Comparison](/docs/comparison) page for detailed feature comparison.

### How to install LTCLAW-GY.X

LTCLAW-GY.X supports multiple installation methods. See
[Quick Start](https://ltclaw-gy-x.agentscope.io/docs/quickstart) for details:
...
cd .. && mkdir -p src/ltclaw-gy-x/console
...
5. If using the Desktop app (Windows `.exe` or macOS `.zip`), you currently need to uninstall and reinstall:
   - Uninstall LTCLAW-GY.X on your machine
   - Download the latest build: https://ltclaw-gy-x.agentscope.io/downloads
   - Reinstall

After upgrading, restart the service with `ltclaw-gy-x app`.

If you previously used LTCLAW-GY.X, upgrading to LTCLAW-GY.X only requires downloading the latest LTCLAW-GY.X. No extra migration is needed; your configuration, memory, skills, and other data from the LTCLAW-GY.X era continue to work.

### How to initialize and start LTCLAW-GY.X service
...
[Quick Start](https://ltclaw-gy-x.agentscope.io/docs/quickstart).

### Port 8088 conflict on Windows

On Windows, Hyper-V and WSL2 may reserve certain port ranges, which can conflict
with LTCLAW-GY.X's default port **8088**. This affects all installation methods
...
- LTCLAW-GY.X fails to start, or browser cannot connect to `http://127.0.0.1:8088/`
...
1. Run `ltclaw-gy-x app --port 8090` from a terminal instead
...
LTCLAW-GY.X is open source. Official repository:
`https://github.com/agentscope-ai/LTCLAW-GY.X`

### Where to check latest version upgrade details

See the site [Release notes](https://ltclaw-gy-x.agentscope.io/release-notes/?lang=en)
or LTCLAW-GY.X GitHub [Releases](https://github.com/agentscope-ai/LTCLAW-GY.X/releases).

### How to configure models

In Console, go to **Settings → Models** to configure. See the
[Models](https://ltclaw-gy-x.agentscope.io/docs/models) doc for details:
...
You can also use `ltclaw-gy-x models` for setup, downloads, and switching. See
[CLI → Models and environment variables → ltclaw-gy-x models](https://ltclaw-gy-x.agentscope.io/docs/cli#ltclaw-gy-x-models).

### How to use LTCLAW-GY.X-Flash series models

LTCLAW-GY.X-Flash is a family of models tuned by the LTCLAW-GY.X team for LTCLAW-GY.X's core
...
LTCLAW-GY.X-Flash models are currently open-sourced on
...
All built-in local providers in LTCLAW-GY.X can be used with LTCLAW-GY.X-Flash models:

#### LTCLAW-GY.X Local (llama.cpp)

In the LTCLAW-GY.X Local model interface, simply choose a LTCLAW-GY.X-Flash model to
...
> LTCLAW-GY.X Local is still in beta. Compatibility across different devices and
> runtime stability are still being improved. If you run into issues while
> using it, please open an issue on GitHub.
> If LTCLAW-GY.X Local does not work properly in your environment, we recommend
> deploying LTCLAW-GY.X-Flash with Ollama or LM Studio first.

#### Ollama

1. Download a quantized LTCLAW-GY.X-Flash model from
   ...
   [LTCLAW-GY.X-Flash-4B-Q4_K_M](https://www.modelscope.cn/models/AgentScope/LTCLAW-GY.X-Flash-4B-Q4_K_M).

   - Download with ModelScope CLI:

     ```bash
     modelscope download --model AgentScope/LTCLAW-GY.X-Flash-4B-Q4_K_M README.md --local_dir ./dir
     ```
...
Create a text file named `ltclaw-gy-x-flash.txt` with the following contents. Replace
`/path/to/your/ltclaw-gy-x-xxx.gguf` with the absolute path to the `.gguf` file in
the LTCLAW-GY.X-Flash model repository you downloaded:
...
4. In LTCLAW-GY.X model settings, choose the Ollama provider, then automatically load
   the model on the Models page.

#### LM Studio

1. Follow step 1 in the Ollama section above to download an appropriate
   quantized LTCLAW-GY.X-Flash model.
...
4. In LTCLAW-GY.X model settings, choose the LM Studio provider, then automatically
   load the model on the Models page.

### When using models deployed with Ollama / LM Studio, why can't LTCLAW-GY.X complete multi-turn interactions, complex tool calls, or remember earlier instructions?

In most cases, this is not a LTCLAW-GY.X bug. The root cause is usually that the
model's context length is configured too small.

When you deploy a local model with Ollama or LM Studio, if the model's
`context length` is too low, LTCLAW-GY.X may show problems such as:
...
- Before running LTCLAW-GY.X, set the model's `context length` to **at least 32K**
- For more complex tasks, frequent tool calls, or longer conversations, you
  may need a value **higher than 32K**

> ⚠️ **Before running LTCLAW-GY.X, you must set the context length to 32K or higher**
>
> For local models deployed with Ollama or LM Studio, LTCLAW-GY.X typically needs a
...
The easiest way to create a cron job is to talk to LTCLAW-GY.X in the channel where you want the results. For example, say: “Create a scheduled task that reminds me to drink water every five minutes.” You can then see the enabled job in Console.

If a scheduled task does not run as expected, try the following:

1. Confirm that the LTCLAW-GY.X service is running.
...
6. To verify that the task was created and can run, click **Execute Now**. If it works, you should see the reply in the target channel. You can also ask LTCLAW-GY.X: “Trigger the ‘drink water reminder’ task I just created.”
...
[Skills](https://ltclaw-gy-x.agentscope.io/docs/skills).

### How to configure MCP

Go to **Agent -> MCP** in Console. You can enable/disable/delete/create MCP
clients there. See [MCP](https://ltclaw-gy-x.agentscope.io/docs/mcp).
...
LTCLAW-GY.X supports API keys obtained via DashScope Coding Plan. If it still fails,
please check:
...
[issue](https://github.com/agentscope-ai/LTCLAW-GY.X/issues) in the LTCLAW-GY.X GitHub
...
Error: Unknown agent error: AuthenticationError: Error code: 401 - {'error': {'message': "You didn't provide an API key. You need to provide your API key in an Authorization header using Bearer auth (i.e. Authorization: Bearer YOUR_KEY). ", 'type': 'invalid_request_error', 'param': None, 'code': None}, 'request_id': 'xxx'}(Details: /var/folders/.../ltclaw-gy-x_query_error_qzbx1mv1.json)

Please upload that file (e.g. `/var/folders/.../ltclaw-gy-x_query_error_qzbx1mv1.json`)
and also provide your current model provider, model name, and LTCLAW-GY.X version.
