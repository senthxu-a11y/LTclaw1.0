...
This section describes multiple ways to install LTCLAW-GY.X:
...
on Windows). This installs the `ltclaw-gy-x` command.
...
To overwrite existing config, use `ltclaw-gy-x init --force` (you will be prompted).
...
channels, LTCLAW-GY.X will reply there. Otherwise, you can complete this section
...
Then open a new terminal (the installer adds LTCLAW-GY.X to your PATH automatically).
...
>      - Check if `uv` is available: Enter `uv --version` in CMD. If a version number appears, **only configure the LTCLAW-GY.X path**. If you receive `'uv' is not recognized as an internal or external command, operable program or batch file,` configure both paths.
...
>      - LTCLAW-GY.X path: Typically `%USERPROFILE%\.ltclaw-gy-x\bin`.
...
To upgrade, simply re-run the install command. To uninstall, run `ltclaw-gy-x uninstall`.
...
Images are on **Docker Hub** (`agentscope/ltclaw-gy-x`). Image tags: `latest` (stable);
`pre` (PyPI pre-release). Also available on Alibaba Cloud ACR for users in China:
`agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/ltclaw-gy-x` (same tags).
...
memory, and skills are stored in the `ltclaw-gy-x-data` volume; model configurations
and API keys are stored in the `ltclaw-gy-x-secrets` volume; backup archives are stored in the `ltclaw-gy-x-backups` volume. To pass API keys, add
...
To deploy LTCLAW-GY.X on Alibaba Cloud, use the ECS one-click deployment:

1. Open the [LTCLAW-GY.X Alibaba Cloud ECS deployment link](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) and fill in the deployment parameters as prompted;
...
For detailed steps and instructions, see [Alibaba Cloud Developer Community: Deploy your AI assistant with LTCLAW-GY.X in 3 minutes](https://developer.aliyun.com/article/1713682).

---

## Option 5: ModelScope Studio one-click setup (no installation)

If you don't want to install Python locally, you can deploy LTCLAW-GY.X to the cloud
through ModelScope Studio:

1. First go to [ModelScope](https://modelscope.cn/register?back=%2Fhome) to register and log in;
2. Open [LTCLAW-GY.X Studio](https://modelscope.cn/studios/fork?target=AgentScope/LTCLAW-GY.X) and complete the one-click setup.

**Important**: Set your Studio to **non-public**, or others may control your LTCLAW-GY.X.
...
LTCLAW-GY.X's desktop application without manually configuring Python environments
or running commands.
...
1. **Download the installer**
   Go to [GitHub Releases](https://github.com/agentscope-ai/LTCLAW-GY.X/releases) to download the version for your system:

   - Windows: `LTCLAW-GY.X-Setup-<version>.exe`
   - macOS: `LTCLAW-GY.X-<version>-macOS.zip`

2. **Install and launch**

   - **Windows**: Double-click the `.exe` file to install following the wizard, then double-click the desktop shortcut to launch
   - **macOS**: Extract the `.zip` to get `LTCLAW-GY.X.app`, first time requires right-click and select "Open" to bypass system security restrictions
...
LTCLAW-GY.X needs a large language model to work. You can choose either option:
...
- LTCLAW-GY.X Local (llama.cpp): download `llama.cpp` inside LTCLAW-GY.X Local provider settings, see [Models - Local providers Configuration](./models) for details.
...
- For LTCLAW-GY.X Local (llama.cpp), you can download models directly from the provider settings in the Console, or manually place GGUF model files in the local models directory (default `~/.ltclaw-gy-x/local_models/models/<org>/<model>`, for example: `~/.ltclaw-gy-x/local_models/models/Qwen/Qwen3-0.6B-GGUF`).
- For Ollama and LM Studio, you need to add models in their respective services first, then LTCLAW-GY.X can automatically fetch the model list and connect to them.
...
page to test functionality and confirm LTCLAW-GY.X can reply normally.
...
Chat with LTCLAW-GY.X in DingTalk, Feishu, QQ, Discord, iMessage, and other apps:
...
4. After saving, you can send messages to LTCLAW-GY.X in the corresponding app

#### 🔧 Enable and extend skills

Give LTCLAW-GY.X more capabilities (PDF processing, Office documents, news summaries, etc.):
...
Let LTCLAW-GY.X execute tasks automatically:

- **Scheduled tasks**: Create in Console **Control → Scheduled tasks**, or use `ltclaw-gy-x cron` command in [CLI](./cli)
