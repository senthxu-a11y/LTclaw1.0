...
LTCLAW-GY.X 支持**多智能体**，允许您在同一个 LTCLAW-GY.X 实例中运行多个独立的 AI 智能体。
...
简单来说，**多智能体**就是让您可以在一个 LTCLAW-GY.X 中运行多个"分身"，每个分身：
...
启动 LTCLAW-GY.X 后，在控制台**左上角**可以看到**智能体切换器**：
...
**注意**：删除后工作区目录会保留（防止误删数据），如需彻底清理，请手动删除 `~/.ltclaw-gy-x/workspaces/{agent_id}` 目录。
...
如果您之前使用 LTCLAW-GY.X **v0.0.x**，升级到 **v0.1.0** 时会**自动迁移**：
...
- `ltclaw-gy-x channels` - 频道管理
- `ltclaw-gy-x cron` - 定时任务
- `ltclaw-gy-x daemon` - 运行状态
- `ltclaw-gy-x chats` - 对话管理
- `ltclaw-gy-x skills` - 技能管理

**不支持 `--agent-id` 的命令**（全局操作）：

- `ltclaw-gy-x init` - 初始化
- `ltclaw-gy-x providers` - 模型提供商
- `ltclaw-gy-x models` - 模型配置
- `ltclaw-gy-x env` - 环境变量
