...
- **工作目录：** 直接在 `$QWENPAW_WORKING_DIR`（默认 `~/.ltclaw-gy-x`）下编辑技能文件，
...
LTCLAW-GY.X 的 skills 分为两层：

- **技能池：** 共享本地仓库，路径是 `$QWENPAW_WORKING_DIR/skill_pool/`
  （默认 `~/.ltclaw-gy-x/skill_pool/`）。
- **工作区技能副本：** 某个工作区真正运行时使用的本地副本，路径是
  `$QWENPAW_WORKING_DIR/workspaces/{agent_id}/skills/`
  （默认 `~/.ltclaw-gy-x/workspaces/{agent_id}/skills/`）。
...
- **冲突：** 如果保存、导入、上传或广播后会落到一个已经存在的名字上，
  LTCLAW-GY.X 不会静默覆盖，而是直接返回冲突。界面 / API 会同时给出一个建议的新名
  字，便于你按这个名字重试。
...
1. **导入内置技能**。
   ...
   | **QA_source_index**           | LTCLAW-GY.X 自身源码与文档的快速索引技能，用于把关键词映射到本地源码路径和文档。                       | 自建                                                           |
   | **cron**                      | 定时任务管理。通过 `ltclaw-gy-x cron` 或控制台定时任务创建、查询、暂停、恢复、删除定时任务。           | 自建                                                           |
   ...
   | **guidance**                  | 回答 LTCLAW-GY.X 安装与配置问题，优先查本地文档。                                                      | 自建                                                           |
   ...
   `ltclaw-gy-x cron` 或控制台 **控制 → 定时任务** 管理：

   - 创建任务：`ltclaw-gy-x cron create --type agent --name "xxx" --cron "0 9 * * *" ...`
   - 查看列表：`ltclaw-gy-x cron list`
   - 查看状态：`ltclaw-gy-x cron state <job_id>`
...
依赖外部二进制或环境变量，可在 `metadata.requires` 中声明；LTCLAW-GY.X 会将其透出为
...
展示字段。当某个 Skill 在当前 workspace 和频道下生效时，LTCLAW-GY.X 会在该次 Agent
...
对于 `requires` 元数据，解析器按顺序检查：`metadata.openclaw.requires` → `metadata.ltclaw-gy-x.requires` → `metadata.requires`，取第一个找到的。
