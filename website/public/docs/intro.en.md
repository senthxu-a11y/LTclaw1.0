...
This page explains what LTCLAW-GY.X is, what it can do, and how to get started
...
LTCLAW-GY.X is a **personal assistant** that runs in your own environment.
...
LTCLAW-GY.X is built by the [AgentScope team](https://github.com/agentscope-ai) on
...
## How do you use LTCLAW-GY.X?

There are two main ways to use LTCLAW-GY.X:

1. **Chat in messaging apps**
   Send messages in DingTalk, Feishu, WeChat, Discord, Telegram, or other apps,
   and LTCLAW-GY.X replies in the same app. Whether looking things up, managing todos,
   or answering questions — it all depends on your enabled Skills. A single
   LTCLAW-GY.X instance can connect to multiple apps, responding in whichever channel
   you're using.

2. **Scheduled execution**
   No need to send messages manually each time. LTCLAW-GY.X can run automatically at
   set times:
   - Send a fixed message to a channel (e.g., "Good morning" to DingTalk at 9am);
   - Ask LTCLAW-GY.X a question and send the answer to a channel (e.g., every 2 hours
     ask "What are my todos?" and post to DingTalk);
   - Run a scheduled check-in or digest: use your prepared questions to query
     LTCLAW-GY.X and send the answers to your last active channel.

After installation, connecting at least one channel, and starting the service,
you can chat with LTCLAW-GY.X in DingTalk, Feishu, QQ, and other apps, and enjoy
...
- **Console** — LTCLAW-GY.X's built-in web management interface. Chat, configure
  channels, manage skills, set up models, and more. See [Console](./console).
- **Channel** — Where you talk to LTCLAW-GY.X (DingTalk, Feishu, QQ, Discord,
  iMessage, etc.). Configure step by step in [Channels](./channels).
- **Heartbeat** — Ask LTCLAW-GY.X your prepared questions at fixed intervals, and
...
- **Scheduled tasks** — Multiple tasks, each with independent time configuration
  (send X daily at 9am, ask LTCLAW-GY.X Y every 2 hours, etc.), managed through the
  Console or [CLI](./cli).
...
- **MCP and tools** — MCP (Model Context Protocol) is a standard protocol for
  connecting external tool servers to extend capabilities. Tools are LTCLAW-GY.X's
  built-in core abilities (reading/writing files, executing commands, browser,
  etc.). See [MCP and Tools](./mcp).
- **Agent/Workspace** — Starting from v0.1.0, LTCLAW-GY.X supports multi-agent,
...
- **Security mechanisms** — LTCLAW-GY.X provides multi-layer security, including
...
2. **[Console](./console)** — After the service starts, open the Console in
   ...
   configuring and using LTCLAW-GY.X.** Start by chatting and configuring models in
   the Console to understand how LTCLAW-GY.X works.
3. **[Models](./models)** — Configure API keys for cloud LLM providers or
   download local models. This is a **required prerequisite** for using LTCLAW-GY.X.
4. **Configure and use as needed**:
   - [Channels](./channels) — Connect DingTalk / Feishu / WeChat / Discord / Telegram to chat with LTCLAW-GY.X in those apps;
   - [Skills](./skills) — Understand and extend LTCLAW-GY.X's capabilities;
   ...
   - [Scheduled tasks](./console#scheduled-tasks) or [CLI](./cli) — Scheduled tasks, `ltclaw-gy-x doctor` diagnostics, `ltclaw-gy-x doctor fix`, and more;
